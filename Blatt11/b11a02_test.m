...
%% Network
vecEqual(net.layers{1}.dimensions, sol.net.layers{1}.dimensions);
assert(net.trainParam.epochs == sol.net.trainParam.epochs, 'The number of training epochs is different from the exercise description');
assert(strcmp(net.layers{1}.topologyFcn, sol.net.layers{1}.topologyFcn), 'The topology of the network object is different from the exercise description');

%% Maps
matEqual(net.IW{1}, sol.net.IW{1});
matEqual(maps, sol.maps);
matEqual(hits, sol.hits);
matEqual(mapSurvived, sol.mapSurvived);
...
function [] = matEqual(matStudent, matSolution)
    ...
    assert(all(all(all(abs(matStudent - matSolution) <= tol))), [sprintf('The values of the matrix %s are not correct:\nactual:\n', name),...
